		...
		for {
			...
			if err == io.EOF {
				...
				if !done {
					...
					// ConPTY (conhost) has closed the output pipe — this happens
					// when a TUI app (e.g. opencode) exits on Windows. The pipe
					// is permanently dead; further reads will only return EOF.
					// pwsh will hang trying to write to the dead ConPTY and
					// eventually crash (0x80131623). Kill and auto-restart.
					...
					go func() {
						...
						if err != nil {
							...
							bc.UpdateControllerAndSendUpdate(func() bool {
								return bc.ProcStatus == Status_Done
							})
						} else {
