...
You can upload local files into the current directory view in two ways:

- Drag files from your OS file manager (Finder / Explorer / etc.) and drop them into the directory preview list.
- Right click in the directory view and choose **Upload Files...**.

This works for both local and remote directory views. For remote views, the files are uploaded to the currently opened remote directory.

##### Copy and Paste Files/Folders

You can copy and paste a file or folder directly from directory context menus:

- Right click a file/folder row and choose **Copy File/Folder**.
- Right click in the destination directory view and choose **Paste**.

This works across local and remote directory views. If you paste into the same directory, TideTerm automatically creates a duplicate name (`copy`, `copy 2`, etc.) instead of overwriting the original.
